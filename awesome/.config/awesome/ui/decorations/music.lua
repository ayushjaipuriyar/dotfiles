...
local function volume_control()
	...
	volume_bar:connect_signal("button::update", function()
		volume_info()
	end)
	...
	-- awful.spawn.easy_async_with_shell(
	--     "ps x | grep \"mpc idleloop mixer\" | grep -v grep | awk '{print $1}' | xargs kill", function()
		...
		if button == 1 then
			awful.spawn.with_shell(
				"pactl set-sink-volume @DEFAULT_SINK@ "
					.. tostring(math.ceil(lx * 100 / volume_bar.forced_width) .. "%")
			)
		end
		volume_info()
	end)
	...
	volume:buttons(gears.table.join( -- Scroll - Increase or decrease volume
