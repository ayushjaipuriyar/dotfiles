...
return function()
	...
	awesome.connect_signal("signal::battery", function(value, state, timeEmpty, timeFull)
		battery_bar.value = value
		last_value = value
		battery_percentage_text:set_text(math.floor(value) .. "%")

		if state == 1 then
			local hours = math.floor(timeFull / 3600)
			local min = math.floor((timeFull - hours * 3600) / 60)
			if min < 10 then
				min = "0" .. min
				time_text:set_text(hours .. ":" .. min)
			else
				time_text:set_text(hours .. ":" .. min)
			end
		else
			local hours = math.floor(timeEmpty / 3600)
			local min = math.floor((timeEmpty - hours * 3600) / 60)
			if min < 10 then
				min = "0" .. min
				time_text:set_text(hours .. ":" .. min)
			else
