...
local update = function()
	awful.spawn.easy_async("pactl get-sink-mute @DEFAULT_SINK@ | awk '{print $2}'", function(stdout)
		if stdout:match("yes") then
			actual.image = icons.mutevolume
		else
			actual.image = icons.volume
		end
	end)
end
...
awesome.connect_signal("module::volume_osd:show", function(bool)
	...
	if bool then
		...
		awesome.emit_signal("module::mic_osd:show", false)
